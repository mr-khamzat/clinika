/**
 * AttributionSection — распределение записей и выручки по источникам/каналам.
 *
 * Поля от бэкенда: channel, total_appointments, unique_patients,
 * first_time_patients, revenue. Бэкенд уже сортирует по выручке.
 */
...
export default function AttributionSection({ clinicId = '' }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await api.get('/manager/analytics/attribution', { params })
  ...
  const summary = useMemo(() => {
    ...
    const first = rows.reduce((s, r) => s + (r.first_time_patients || 0), 0)
    const revenue = rows.reduce((s, r) => s + (r.revenue || 0), 0)
    return { channels, first, revenue }
  }, [rows])
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 12, fontSize: 13, color: 'var(--fg-3)' }}>
        Распределение записей и выручки по источникам/каналам
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <KpiCard label="Всего источников" value={fmt(summary.channels)} />
        <KpiCard label="Всего первичных" value={fmt(summary.first)} />
        <KpiCard label="Общая выручка" value={money(summary.revenue)} />
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--bg-1)' }}>
                ...
                <th style={{ ...th(), textAlign: 'right' }}>Первичных</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} style={{ padding: 24 }}>
                  <EmptyState icon="hub" title="Нет данных" text="Источники для записей не заполнены или нет записей за период." />
                </td></tr>
              ) : rows.map((r, i) => {
                const firstColor = r.first_time_patients > 0 ? 'var(--good)' : 'var(--fg-3)'
                return (
                  <tr key={`${r.channel}-${i}`} style={{ borderTop: '1px solid var(--border)' }}>
                    <td style={{ ...td(), fontWeight: 600 }}>{r.channel || '—'}</td>
                    <td style={{ ...td(), textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{fmt(r.total_appointments)}</td>
                    <td style={{ ...td(), textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(r.unique_patients)}</td>
                    <td style={{ ...td(), textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: firstColor, fontWeight: r.first_time_patients > 0 ? 600 : 400 }}>
                      {fmt(r.first_time_patients)}
                    </td>
