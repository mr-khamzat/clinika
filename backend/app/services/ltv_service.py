...
Архитектура:
  ...
  RenovatioAdapter   — обёртка над mis_client.get_appointments
...
Ключевые правила:
  ...
  - LTV горизонт = 3 года: ltv = avg_check × visits_per_year × 3
...
class RenovatioAdapter:
    """Обёртка над mis_client.get_appointments. Группирует визиты по телефону."""
        ...
        for mis_clinic_id in target_mis_ids:
            try:
                items = await get_appointments(
                    int(mis_clinic_id), date_from, date_to,
                ...
                log.warning("ltv: tenant=%s mis_clinic=%s ошибка: %s", tenant.slug, mis_clinic_id, e)

        # 4. Преобразуем в VisitRecord (только «состоявшиеся»: completed/выполнено/status_id=4)
...
async def compute_cohorts(
    ...
    Возвращает [{cohort, patients, total_spent, avg_ltv}].
    """
