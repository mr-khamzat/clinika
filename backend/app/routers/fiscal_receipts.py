...
async def upsert_ofd_config(
    ...
    # Колонка в модели — api_key_encrypted; чтение — через OFDConfig.decrypted_api_key.
    ...
    if cfg is None:
        ...
        cfg = OFDConfig(
            ...
            inn=body.inn,
            api_key_encrypted=_encrypt_secret(body.api_key),
        ...
        cfg.inn = body.inn
        if body.api_key:
            cfg.api_key_encrypted = _encrypt_secret(body.api_key)
