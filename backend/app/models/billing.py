...
class Invoice(Base):
    ...
    # B2B акты оказанных услуг (Phase 1 SaaS)
    act_number: Mapped[str | None] = mapped_column(String(50), nullable=True, index=True)
    act_status: Mapped[str] = mapped_column(String(30), nullable=False, default="draft")
    act_type: Mapped[str] = mapped_column(String(30), nullable=False, default="subscription")
    ...
    act_pdf_path: Mapped[str | None] = mapped_column(String(500), nullable=True)
    act_line_items: Mapped[list | None] = mapped_column(JSONB, nullable=True, default=list)
    subtotal: Mapped[Decimal | None] = mapped_column(Numeric(14, 2), nullable=True)
    tax_rate: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True, default=Decimal("0"))
    tax_amount: Mapped[Decimal | None] = mapped_column(Numeric(14, 2), nullable=True, default=Decimal("0"))
    total: Mapped[Decimal | None] = mapped_column(Numeric(14, 2), nullable=True)
    legal_entity_name: Mapped[str | None] = mapped_column(String(300), nullable=True)
    legal_entity_inn: Mapped[str | None] = mapped_column(String(20), nullable=True)
    ...
    soft_lock_applied_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
